import React, { Component } from 'react';
import {NavLink} from 'react-router-dom';
import './funny2.css';
class game8 extends Component {
    render() {
        return (
            <div className="game8">
                 {/* <div className="container" style={{opacity:0.7,marginLeft:"10%"}}>
                <h1 style={{paddingTop:'20px',backgroundColor:'pink',color:'blue'}}>LEVEL 5: LAST LEVEL</h1>  
                </div>   */}
            <div className="funny5a">
            <NavLink to="/finalscreen">
            <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
            </NavLink>
            </div>
            <div className="funny5b">
                
                    <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
            
            </div>

            <div className="funny5c">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>
        <div className="funny5d">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>
        <div className="funny5e">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>
        <div className="funny5f">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>
        <div className="funny5g">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>
        <div className="funny5h">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>
        <div className="funny5i">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>
        <div className="funny5k">
                
                <button type="button" className="btn btn-success btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-up" /> HAPPY</button>
        
        </div>

        <div className="funny5ado">
                <NavLink to="/game7">
                <button type="button" className="btn btn-danger btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-down" /> SAD</button>
                </NavLink>
        </div>
        <div className="funny5bdo">
                <NavLink to="/game7">
                <button type="button" className="btn btn-danger btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-down" /> SAD</button>
                </NavLink>
        </div>
        <div className="funny5cdo">
                <NavLink to="/game7">
                <button type="button" className="btn btn-danger btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-down" /> SAD</button>
                </NavLink>
        </div>
        <div className="funny5ddo">
                <NavLink to="/game7">
                <button type="button" className="btn btn-danger btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-down" /> SAD</button>
                </NavLink>
        </div>
        <div className="funny5fdo">
                <NavLink to="/game7">
                <button type="button" className="btn btn-danger btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-down" /> SAD</button>
                </NavLink>
        </div>
        <div className="funny5gdo">
                <NavLink to="/game7">
                <button type="button" className="btn btn-danger btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-down" /> SAD</button>
                </NavLink>
        </div>
        <div className="funny5hdo">
                <NavLink to="/game7">
                <button type="button" className="btn btn-danger btn-lg btn3d" ><span className="glyphicon glyphicon-thumbs-down" /> SAD</button>
                </NavLink>
        </div>
            </div>
        );
    }
}



export default game8;